...
# Test cited in manuscript for APOBEC...
...
# ... and SBS12
signature_comparisons[signature == 'SBS12'][comparison %in% c('IHC-PHC', 'DCC-PHC'), .(comparison, pval)]
# comparison       pval
# <char>      <num>
#   1:    IHC-PHC 0.01470659
# 2:    DCC-PHC 0.02295139
...
# Add composite signatures to raw_mp_out
raw_mp_out[, APOBEC := SBS2 + SBS13]
raw_mp_out[, Treatment := SBS32 + SBS86 + SBS87 + SBS99]

raw_mp_out = raw_mp_out[, .SD, .SDcols = c('Unique_Patient_Identifier', 'boot',
                                           setdiff(sig_cols, c('Treatment', 'APOBEC')), 'APOBEC', 'Treatment')]
...
# No longer need to handle APOBEC and Treatment separately for putative detection
# since they're already in the melted data
...
# Update signature_names to include all signatures we're tracking
signature_names = unique(sig_cols)
