...
saveRDS(path_grl_exome, 'output/pathway/pathway_consensus_exome_granges.rds', 
        compress = 'xz')
saveRDS(path_grl_mskex, 'output/pathway/pathway_consensus_msk-exome_granges.rds', 
        compress = 'xz')
...
get_pathway_gene_anno = function(defs) {
  path_to_variants = unique(rbindlist(lapply(defs, as.data.table), idcol = 'path_id'))
  setnames(path_to_variants, 'V1', 'variant_id')
  anno = select_variants(cesa, variant_ids = unique(path_to_variants$variant_id), 
                         include_subvariants = T)[variant_type == 'snv']
  path_to_variants[anno, let(maf_prevalence = maf_prevalence, gene = gene),
                   on = 'variant_id']
  return(path_to_variants)[]
}


cancer_pw_genes = get_pathway_gene_anno(present_variants_mskex)

# 299 cancer genes to start
cancer_pw_genes = cancer_pw_genes[maf_prevalence > 0]
stopifnot(uniqueN(cancer_pw_genes$gene) == 299) 
min_genes_per_pw = 3
eligible_cancer_pw = cancer_pw_genes[, .(num_genes = uniqueN(gene)), by = 'path_id'][num_genes >= min_genes_per_pw, path_id]
cancer_pw_genes = cancer_pw_genes[path_id %in% eligible_cancer_pw]

stopifnot(uniqueN(cancer_pw_genes$gene) == 294) # kept most genes
cancer_pw_to_try = unique(cancer_pw_genes$path_id)
...
redefine_pathways = function(pathway_defs, samples_to_use, freq_threshold, cores = 1, stop_at = Inf) {
  ...
  while(TRUE) {
    ...
    num_genes_by_path = get_pathway_gene_anno(needing_mut_freq_check)[maf_prevalence > 0, .(num_gene = uniqueN(gene)), by = 'path_id']
    too_few_genes = num_genes_by_path[num_gene < min_genes_per_pw, path_id]
    
    pboptions(prev_opt)
    bad_paths = unique(c(bad_paths, names(path_mut_freq[path_mut_freq < freq_threshold]), too_few_genes))
    num_bad = length(bad_paths)
    if(num_bad > 0) {
      message("Removed ", num_bad, " pathways due to low frequency of mutation or too few genes.")
...
freq_threshold = .05
initial_cancer_pw = present_variants_mskex[names(present_variants_mskex) %in% cancer_pw_to_try]
names(initial_cancer_pw) = paste0(names(initial_cancer_pw), '.tgs')
final_cancer_pw = redefine_pathways(pathway_defs = initial_cancer_pw, 
                                    samples_to_use = cesa$samples$Unique_Patient_Identifier, 
                                     freq_threshold = freq_threshold, cores = 2, stop_at = 20)

# We got 182 cancer genes represented in the top 20 cancer gene pathways
stopifnot(uniqueN(get_pathway_gene_anno(final_cancer_pw$defs)$gene) == 182)

already_used = unname(unlist(final_cancer_pw$defs))
present_variants_by_path_exome = present_variants_by_path[! paste0(names(present_variants_by_path), '.tgs') %in% 
                                                      names(final_cancer_pw$defs)]
exome_pathway_defs = lapply(present_variants_by_path_exome, function(x) setdiff(x, already_used))
exome_pathway_defs = exome_pathway_defs[sapply(exome_pathway_defs, length) > 0]
...
final_pathway_defs = c(final_cancer_pw$defs, exome_pw_redefined$defs)
final_effects = rbind(final_cancer_pw$effects, exome_pw_redefined$effects)
setnames(final_effects, 'variant_name', 'path_id')

final_path_to_variants = get_pathway_gene_anno(final_pathway_defs)
...
custom_pw_info[, is_cancer_gene_path := weighted_prop_msk == 1]
