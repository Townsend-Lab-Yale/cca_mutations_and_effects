...
major_sigs = for_plot_whole[, .(highest_contribution = max(contribution, na.rm = T)),
                            by = 'short_signature'][order(-highest_contribution), short_signature][1:16]

for_plot = for_plot_whole[short_signature %in% major_sigs]

# exclude individual APOBEC and treatment-associated signatures, if present
for_plot = for_plot[!short_signature %in% c('2', '13', '32', '86', '87', '99')]
...
sample_counts_by_type[, count_label := paste0('(<i>n</i> = ', N, ')')]
for_plot[sample_counts_by_type, count_label := count_label, on = 'cca_type' ]

# Build subtype labels
for_plot[, type_label := fcase(
  cca_type == 'IHC', paste0('Intrahepatic<br>', count_label),
  cca_type == 'PHC', paste0('Perihilar<br>', count_label),
  cca_type == 'DCC', paste0('Distal<br>', count_label),
  cca_type == 'EHC', paste0('Extrahepatic<br>', count_label)
)]
...
signature_labels = c(
  "1"  = "Clock-like (1)",
  "5"  = "Clock-like (5)",
  "3"  = "Defective homologous\nrecombination (3)",
  "15" = "Mismatch repair defects (15)",
  "APOBEC" = "APOBEC (2, 13)",
  "Treatment" = "Treatment (32, 86, 87, 99)",
  "22a" = "Aristolochic acid exposure (22a)",
  "24" = "Aflatoxin exposure (24)",
  "42" = "Haloalkane exposure (42)",
  "16" = "Alcohol (16)",
  "12" = "Unknown, liver cancer-associated (12)",
  "40a" = "Unknown (40a)",
  "40c" = "Unknown (40c)"
)

custom_order = rev(names(signature_labels))

for_plot = for_plot[short_signature %in% custom_order]
for_plot[, short_signature := factor(short_signature, levels = custom_order)]
...
dotplot = ggplot(data = for_plot, 
  ...
  geom_text(data = signif_labels, aes(label = signif_label, x = x, y = short_signature), 
            nudge_y = -0.15, size = base_text_size/2, na.rm = TRUE, inherit.aes = FALSE) +
  ...
  theme_bw() +
  theme(
    axis.text.x = element_markdown(size = base_text_size),
    ...
    plot.title = element_text(size = base_text_size),
    panel.grid.major.x = element_blank(), 
    panel.grid.major.y = element_line(linewidth = 0.05, color = "grey"),
    panel.border = element_blank(), 
    axis.ticks.y = element_blank()
  ) +
  scale_y_discrete(labels = signature_labels)
...
# A manuscript claim: 
stopifnot(min(prop_represented$total) > .80)

# save plot
ggsave(file = 'figures/prepped_figures/signature_attributions.png', plot = dotplot, height = 5.5, width = 10, units = 'in')
