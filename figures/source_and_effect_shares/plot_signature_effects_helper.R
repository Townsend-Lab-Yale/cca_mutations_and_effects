...
plot_cca_signature_effects = function(mutational_effects = NULL,
  ...
  gg = ggplot(data = df_final) +
    ...
    theme_classic() + theme(
      ...
      strip.text.y.left = element_text(size = base_text_size, angle = 0)
    ) +
  ...
  gg = gg + guides(fill = guide_legend(reverse=TRUE, override.aes = list(size = 2)))
  return(gg)
}

plot_cca_signature_effects_three_panel = function(mutational_effects = NULL,
                                                  signature_groupings = 'auto',
                                                  viridis_option = NULL,
                                                  num_sig_groups = 7, 
                                                  other_color = 'white',
                                                  legend = NULL,
                                                  panel_widths = c(2, 0.01, 1, 0.01, 4, 0.01, 2)) {
  if(! require("ggplot2")) {
    stop('Package ggplot2 must be installed for plotting.')
  }
  
  if(! require("patchwork")) {
    stop('Package patchwork must be installed for combining plots.')
  }
  ...
  # Handle signature groupings (same logic as original function)
  if (running_cannataro) {
    signature_groupings = list(
      "Deamination with age, clock-like (1)" = "SBS1",
      "Unknown, clock-like (5)" = "SBS5",
    ...
    signature_groupings_orig = signature_groupings[name %in% shares_melted$name]
    signature_groupings_orig[is.na(short_name), short_name := '']
    signature_groupings_orig[is.na(description) | description == '',
                             let(description = short_name, short_name = '')]
    
    shares_melted[signature_groupings_orig, c('description', 'short_name') := .(description, short_name), on = 'name']
    shares_melted = shares_melted[! is.na(short_name)]
    shares_melted[, sig_grp_id := .GRP, by = 'description']
    summed_shares = shares_melted[, .(in_grp_sum = sum(value), description = description[1]), by = c('cohort', 'sig_grp_id')]
  ...
  # Create signature labels
  signature_labels = rbindlist(lapply(1:length(signature_groupings), 
                                      function(x) data.table(name = signature_groupings[[x]], 
                                                             label = names(signature_groupings)[x])))
  df_final[signature_labels, group := label, on = 'name']
  
  # Define panel categories - match your actual group names
  # Look for Panel B groups: Source share < effect share in ALL subtypes
  treatment_groups = unique(df_final$group)[grepl("Chemotherapeutic|Treatment|treatment", unique(df_final$group), ignore.case = TRUE)]
  # as well as mismatch repair and aristolochic acid
  mismatch_repair_groups = unique(df_final$group)[grepl("Mismatch", unique(df_final$group), ignore.case = TRUE)]
  aristolochic_acid_groups = unique(df_final$group)[grepl("Aristolochic", unique(df_final$group), ignore.case = TRUE)]
  tobacco_groups = unique(df_final$group)[grepl("Tobacco", unique(df_final$group), ignore.case = TRUE)]
  sbs3_group = unique(df_final$group)[grepl("Defective", unique(df_final$group), ignore.case = TRUE)]
  sbs5_group = unique(df_final$group)[grepl("Unknown", unique(df_final$group), ignore.case = TRUE)]
  aflatoxin_group = unique(df_final$group)[grepl("Aflatoxin", unique(df_final$group), ignore.case = TRUE)]
  panel_B_groups = c(treatment_groups, mismatch_repair_groups, aristolochic_acid_groups, tobacco_groups, sbs3_group, sbs5_group, aflatoxin_group)

  # Look for Panel A groups: Source share > effect share in ALL subtypes
  sbs1_groups = unique(df_final$group)[grepl("Deamination.*age|clock.*like.*1", unique(df_final$group), ignore.case = TRUE)]
  apobec_groups = unique(df_final$group)[grepl("APOBEC", unique(df_final$group), ignore.case = TRUE)]
  panel_A_groups = c(sbs1_groups, apobec_groups)
  
  # Add panel assignment
  df_final[, panel := ifelse(group %in% panel_A_groups, "A",
                             ifelse(group %in% panel_B_groups, "B", "C"))]
  
  # Prepare data formatting (same as original)
  df_final$facet = factor(df_final$sample_group, levels = sample_groupings)
  df_final$weight_type = ifelse(df_final$type == "SW", "Source\nshare", "Effect\nshare")
  df_final$weight_type = factor(df_final$weight_type, levels = c("Effect\nshare", "Source\nshare"))
  
  # Order groups within each panel
  summed_shares = df_final[type == 'CEW' & group != other_label, .(summed_share = sum(prop)), by = c('group', 'sample_group')]
  mean_share_order = summed_shares[, .(mean_share = mean(summed_share)), by = 'group'][order(mean_share), group]
  ...
  if(using_cannataro_colors) {
    ...
    color_mapping = cannataro_colors
  } else if(is.null(df_final$color)) {
    # Use viridis colors
    n_groups = length(unique(df_final$group))
    viridis_colors = viridis::viridis_discrete(n = n_groups, option = viridis_option, begin = .2, direction = -1)
    color_mapping = setNames(viridis_colors, levels(df_final$group))
  } else {
    df_final[group == other_label, color := other_color]
    color_mapping = setNames(unique(df_final$color), unique(df_final$group))
  }
  
  # Function to create individual panel plot
  create_panel_plot = function(panel_data, panel_id, show_y_labels = FALSE, show_legend = FALSE,
                               subtitle = NULL) {
    max_total_prop = panel_data[, .(total = sum(prop)), by = .(weight_type, facet)][, max(total, na.rm = TRUE)]
    
    xmax_rounded = ceiling(max(0.2, max_total_prop) * 5) / 5
    x_breaks = seq(0, xmax_rounded, by = 0.2)
    
    gg = ggplot(data = panel_data) +
      geom_bar(
        mapping = aes(y = weight_type, fill = group, weight = prop),
        position = 'stack',
        width = .9,
        color = 'black'
      ) +
      ...
      theme(
        ...
        legend.text = element_text(size = base_text_size),
        strip.text.y.left = element_text(size = base_text_size, angle = 0),
        plot.title = element_text(size = base_text_size+2, hjust = 0)
      ) +
      scale_x_continuous(
        breaks = x_breaks,
        limits = c(0, xmax_rounded),
        expand = expansion(add = 0),
        labels = scales::label_number(accuracy = 0.1)
      ) +
      scale_fill_manual(name = 'Signatures (COSMIC SBS)', values = color_mapping) +
      guides(fill = guide_legend(reverse = TRUE)) +
      ggtitle(panel_id)
    if(! is.null(subtitle)) {
      gg = gg + labs(subtitle = subtitle) + theme(plot.subtitle = element_text(size = 9, hjust = .5))
    }
    ...
    if (!any(is.na(panel_data$facet))) {
      gg = gg + facet_wrap(~ facet, ncol = 1, strip.position = 'left') + 
        theme(
          strip.background = element_blank(), 
          strip.placement = 'outside',
          strip.text.y.left = element_text(size = base_text_size)
        )
      
      if (!show_y_labels) {
        gg = gg + theme(strip.text.y.left = element_blank())
      }
    }
  ...
  df_final = df_final[! group %like% c("(Infrequently attributed)|(Unknown etiology)")] # not showing these
  panel_a_data = df_final[panel == "A"]
  panel_b_data = df_final[panel == "B"]
  panel_c_data = df_final[panel == "C"]
  
  plot_a = create_panel_plot(panel_a_data, "", show_y_labels = TRUE, show_legend = FALSE) +
    xlab("Proportion\n(effect share < source share, all subtypes)")
  plot_b = create_panel_plot(panel_b_data, "", show_y_labels = FALSE, show_legend = FALSE) +
    xlab("Proportion\n(effect share > source share, all subtypes)")
  plot_c = create_panel_plot(panel_c_data, "", show_y_labels = FALSE, show_legend = FALSE) +
    xlab("Proportion\n(effect share ≈ source share)")
