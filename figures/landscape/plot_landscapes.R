...
plot_landscape = function(features, sample_key, path_to_use = pw_info$path_id, 
  ...
  dt[, let(x = 1:.N, y = 1)]
  left_label_max_size =  max(nchar(pw_names)) + 1
  ...
  make_row = function(dt, column, label = column, legend_name = label, viridis_c_opt = list(),
    ...
    gg = ggplot(dt[, .(x, y, curr_col, cluster_id)], aes(x, y)) + 
      ...
      scale_y_continuous(breaks = 1,
                         labels = stringr::str_pad(label, left_label_max_size, 'left', use_width = FALSE),
      ...
      theme(plot.margin = margin(0, 0, 0, 0, unit = 'pt'), axis.line = element_blank(), axis.text.x = element_blank(),
            ...
            axis.title.y = element_blank(), axis.text.y = element_text(family = 'Courier', size = 8),
            legend.title = element_text(family = 'Courier', size = 9, face = 'bold'),
...
ggsave('figures/prepped_figures/MSK_landscape.png', msk_landscape, width = 12, height = 8, bg = 'white')
fwrite(msk_landscape$data, 'output/landscape/msk_landscape_plotted_features.txt', sep = "\t")


# We're not saving visuals for other sample groupings, but we are using the landscape data for some survival modeling.
ihc_nontarget_landscape = plot_landscape(features = 'output/landscape/nontarget_ihc_features.txt', sample_key = sample_key,
                                         ...
                                         path_to_use =  pw_info[order(-ihc_si), path_id])
#ggsave('figures/landscape/IHC_nontarget_landscape.pdf', ihc_nontarget_landscape, width = 12, height = 8, device = cairo_pdf)
fwrite(ihc_nontarget_landscape$data, 'output/landscape/ihc_nontarget_landscape_plotted_features.txt', sep = "\t")


pan_landscape_cg = plot_landscape(features = 'output/landscape/pan_landscape_features_cancer_pw.txt',
                                   sample_key = sample_key, row_spec = row_spec[c('age', 'tmb', 'metastatic_disease')],
                                   path_to_use = pw_info[is_cancer_gene_path == T, path_id])
#ggsave('figures/landscape/pan_landscape_cancer_pw.pdf', pan_landscape_cg, width = 12, height = 8, device = cairo_pdf)
fwrite(pan_landscape_cg$data, 'output/landscape/pan_landscape_pw_plotted_features.txt', sep = "\t")
...
nontarget_pan_landscape = plot_landscape(features = 'output/landscape/nontarget_pan_landscape_features.txt',
                               sample_key = sample_key[seq_type != 'TGS'],
                               path_to_use = pw_info$path_id,
                               row_spec = row_spec[c('seq_type', 'cca_subtype', 'age', 'tmb', 'metastatic_disease')])
#ggsave('figures/landscape/nontarget_pan_landscape.pdf', nontarget_pan_landscape, width = 12, height = 8, device = cairo_pdf)
fwrite(nontarget_pan_landscape$data, 'output/landscape/nontarget_pan_landscape_plotted_features.txt', sep = "\t")
